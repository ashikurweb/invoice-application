...
<template>
    <div class="invoices mt-10 p-8 bg-white shadow-xl max-w-6xl mx-auto rounded-xl">
        ...
        <div class="flex justify-between items-center mb-8">
            <div>
                <h2 class="text-4xl font-semibold text-gray-800">New Invoice</h2>
            </div>
        </div>
        ...
        <div class="">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
                ...
                <div>
                    ...
                    <select v-model="customer_id" class="input w-full p-3 border text-slate-950 border-gray-300 rounded-lg outline-none focus:ring-1 focus:ring-blue-500 transition-all duration-300">
            ...
            <div class="table w-full mb-8">
                ...
                <div v-for="(item_cart, i) in list_cart" :key="item_cart.id" class="flex justify-between gap-6 items-center p-4 border-b border-gray-200">
                    <p>#{{ item_cart.item_code }}</p>

                    <!-- Description limited to 5 characters and displayed in one line -->
                    <p class="truncate">{{ item_cart.description.slice(0, 5) }}</p>
                    ...
                    <p v-if="item_cart.quantity">{{ item_cart.quantity * item_cart.unit_price }}</p>
                    <p v-else></p>
                    <p class="text-red-500 text-2xl cursor-pointer" @click="removeItem(i)">&times;</p>
                </div>
            ...
            <div class="table__footer mt-8">
                <div class="document-footer grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div>
                        ...
                        <textarea cols="50" rows="3" v-model="form.terms_and_conditions" class="textarea w-full p-3 border border-gray-300 rounded-lg outline-none focus:ring-1 focus:ring-blue-500 transition-all duration-300"></textarea>
                    </div>
        ...
        <div class="card__header mt-10 flex justify-end">
            <button @click="onSave()" class="btn bg-purple-500 text-white px-8 py-3 rounded-lg hover:bg-transparent hover:text-purple-500 border border-purple-500 transition-all duration-300 cursor-pointer">
                Save Invoice
            </button>
        </div>
    </div>
                    ...
                    <ul class="divide-y divide-gray-200">
                        <li v-for="(item, i) in list_products" :key="item.id" class="flex items-center justify-between text-left p-2 border-b border-gray-200">
                            ...
                            <a href="#" class="flex-1 px-4 text-gray-900 truncate hover:text-blue-500 hover:underline transition-all duration-300">
                                {{ item.item_code }} {{ item.description.split(' ').slice(0, 10).join(' ') }}...
                            </a>
